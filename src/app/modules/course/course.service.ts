...
const assignCourseFacultiesIntoDB = async (courseId: string, payload: Partial<TCourseFaculties>) => {
    const result = await CourseFaculties.findByIdAndUpdate(courseId, { $addToSet: { $each: payload } }, { upsert: true, new: true })
