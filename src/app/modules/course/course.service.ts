...
const updateCourseIntoDB = async (
  ...
  try {
    ...
    if (preRequisiteCourses && preRequisiteCourses?.length > 0) {
      ...
      const deletedPreRequusites = preRequisiteCourses
        .filter((el) => el.course && el.isDeleted)
        .map((el) => el.course);

      const deletedPreRequusitesCourses = await Course.findByIdAndUpdate(
        courseId,
        {
          $pull: {
            preRequisiteCourses: { course: { $in: deletedPreRequusites } },
          },
        },
        { new: true, runValidators: true, session },
      );
      ...
      const newPreRequsitesCourses = await Course.findByIdAndUpdate(
        courseId,
        {
...
const assignCourseFacultiesIntoDB = async (
  ...
  const result = await CourseFaculty.findByIdAndUpdate(
    courseId,
    { course: courseId, $addToSet: { faculties: { $each: payload } } },
